Shader "Custom/MagicTV_Screen_Pro"
{
    Properties
    {
        ...
        _BlackLevel ("Black Balance", Range(0.0, 1.0)) = 0.0
        _WhiteLevel ("White Balance", Range(0.0, 1.0)) = 1.0

        _Contrast ("Contrast", Range(0.5, 2.0)) = 1.0
        _Brightness ("Brightness", Range(-0.5, 0.5)) = 0.0

        _ColorDrift ("Color Drift", Vector) = (0.002, 0.002, 0, 0)

        _ChromaNR ("Chroma NR", Range(0.0, 1.0)) = 0.3

        [Toggle] _Reverse ("Reverse (Nega/Posi)", Float) = 0
        
        _VideoArtColor ("Video Art Color", Color) = (0.0, 0.0, 0.0, 1.0)
        _VideoArtBlend ("Video Art Blend", Range(0.0, 1.0)) = 0.0
        _WipePosition ("Wipe Position", Range(0.0, 1.0)) = 1.0
        _WipeDirection ("Wipe Direction (0=down, 1=up)", Range(0.0, 1.0)) = 1.0

        _WhiteClipMask ("White Clip Amount", Range(0.0, 1.0)) = 0.0
        _BlackClipMask ("Black Clip Amount", Range(0.0, 1.0)) = 0.0
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            CBUFFER_START(UnityPerMaterial)
                ...
                float _ChromaNR;
                float _Reverse;
                float4 _VideoArtColor;
                float _VideoArtBlend;
                float _WipePosition;
                float _WipeDirection;
                float _WhiteClipMask;
                float _BlackClipMask;
                float _CRTWarp;
            ...
            half4 frag(Varyings input) : SV_Target
            {
                ...
                float3 color;
                color.r = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, warpedUV + _ColorDrift).r;
                color.g = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, warpedUV).g;
                color.b = SAMPLE_TEXTURE2D(_MainTex, sampler_MainTex, warpedUV - _ColorDrift).b;
                ...
                Y = (Y - _BlackLevel) / max(_WhiteLevel - _BlackLevel, 0.001);
                ...
                float blackMask = smoothstep(0.0, 0.2, luminance);
                float3 clipColor = yuv2rgb(float3(Y, lerp(UV, float2(0.0, 0.0), _WhiteClipMask * whiteMask + _BlackClipMask * blackMask)));
                rgb = lerp(rgb, clipColor, whiteMask * _WhiteClipMask + blackMask * _BlackClipMask);

                if (_Reverse > 0.5) rgb = 1.0 - rgb;

                rgb = lerp(rgb, _VideoArtColor.rgb, _VideoArtBlend);

                float wipe = _WipeDirection > 0.5 ? uv.y : (1.0 - uv.y);
                float wipeFactor = smoothstep(_WipePosition, _WipePosition + 0.001, wipe);
                rgb = lerp(rgb, _VideoArtColor.rgb, wipeFactor);
